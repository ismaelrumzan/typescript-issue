...
export module Generator {
  export function generate({ doc, ...props }: GenerationProps) {
    ...
    doc.setFontSize(22);
    doc.text('Zwischen oktus', width / 2, 140, { align: 'center' });
    ...
    var text = `Für die Einhaltung der vereinbarten Schutzmaßnahmen und deren geprüfter Wirksamkeit wird auf die vorliegende Zertifizierung nach Art. 42 DS-GVO verwiesen, deren Einhaltung durch den Auftragnehmer am ${new Date().toLocaleDateString(
      'de-DE'
    ...
    doc.text('(2)', 17, 112);
    var text =
      'Im Falle einer Inanspruchnahme des Auftraggebers durch eine betroffene Person hinsichtlich etwaiger Ansprüche nach Art. 82 DS-GVO, gilt §3 Abs. 10 entsprechend. (Anmerkung: Im Vertrag können die Parteien hierzu eine Vergütungsregelung treffen).';
    var wrapped = doc.splitTextToSize(text, width - verticalMargin * 2);
