...
export module Generator {
  export function generate({ doc, ...props }: GenerationProps) {
    ...
    const signatureWidth = 100 / 1.35
    const signatureHeight = 50 / 1.35
    ...
    doc.text(wrapped, horizontalMargin, 43);

    doc.setFont("Helvetica", "bold");
    doc.setFontSize(18);
    var text = "§ 1 Gegenstand, Dauer und Spezifizierung der Auftragsverarbeitung"
    var wrapped = doc.splitTextToSize(text, width - verticalMargin);
    doc.text(wrapped, horizontalMargin, 102);
    ...
    var wrapped = doc.splitTextToSize(text, width - verticalMargin);
    doc.text(wrapped, horizontalMargin, 121);
    ...
    var wrapped = doc.splitTextToSize(text, width - verticalMargin);
    doc.text(wrapped, horizontalMargin, 185);
    ...
    if (props.signature) {
      doc.addImage(props.signature, "JPEG", horizontalMargin, 100, signatureWidth, signatureHeight)
    }

    doc.setFont("Helvetica", "normal");
    doc.setFontSize(12);
    doc.text(`${props.firstName} ${props.lastName}`, horizontalMargin + 10, 100 + signatureHeight + 5)
  }
}
